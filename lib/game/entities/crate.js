...
.defines(function() {
    EntityCrate = ig.Entity.extend({
        ...
        update: function() {
            ...
            // if pushing and collision type is not ACTIVE
            if ( this.pushing && this.collides != ig.Entity.COLLIDES.ACTIVE ) {
                this.collides = ig.Entity.COLLIDES.ACTIVE;
            }
            // if not pushing and collision type is not FIXED
            else if ( ! this.pushing && this.collides != ig.Entity.COLLIDES.FIXED ) {
                this.collides = ig.Entity.COLLIDES.FIXED;
            }
            
            // if falling and collision is not ACTIVE
            //if ( this.vel.y != 0 && this.collides != ig.Entity.COLLIDES.ACTIVE ) {
                //this.collides = ig.Entity.COLLIDES.ACTIVE;
            //}
            
            // if falling and collision is ACTIVE
        ...
        animate: function() {
            ...
            */
            ...
            if ( this.collides == ig.Entity.COLLIDES.ACTIVE ) {
                if ( this.currentAnim != this.anims.active ) {
                    this.currentAnim = this.anims.active.rewind();
                }
            }
            else if ( this.collides == ig.Entity.COLLIDES.FIXED ) {
                if ( this.currentAnim != this.anims.idle ) {
                    this.currentAnim = this.anims.idle.rewind();
                }
            }
            ...
            //}
            //this.currentAnim.alpha = ( this.isPushable ? 1 : 0.5 );
        },
            ...
            if ( this.isPushable ) {
                if ( other instanceof EntityPlayer ) {
                    ...
                    if ( axis == 'x' ) {
                        ...
                        var minY = ( this.pos.y + this.size.y ) - 2;
                        var maxY = ( this.pos.y + this.size.y ) + 2;
                        var posY = ( other.pos.y + other.size.y );
