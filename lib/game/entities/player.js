...
.defines(function() {
    EntityPlayer = ig.Entity.extend({
        ...
        checkStatus: function() {
            ...
            if ( ! this.hurting && ! this.dying && ! this.pushing ) {
        ...
        isCrouching: function() {
            ...
            if ( this.hurting || this.dying || this.jumping || this.falling || this.attacking || this.pushing ) {
                return;
            }
        ...
        isAttacking: function() {
            ...
            if ( this.hurting || this.dying || this.crouching || this.pushing ) {
        ...
        isPushing: function() {
            ...
            if ( this.pushing ) {
                ...
                // update pushed entity's velocity
                if ( this.pushingEntity ) {
                    this.pushingEntity.beingPushed( this );
                }
                
                // if pulling entity to the left
                if ( this.pos.x < this.pushingEntity.pos.x && this.vel.x < 0 ) {
                    
                    // check for a wall to the left
                    // *prevents the entity from being smashed between the wall and pushable entity
                    var xPos = ( this.pos.x - 2 );
                ...
                // else, if pulling entity to the right
                if ( this.pos.x > this.pushingEntity.pos.x && this.vel.x > 0 ) {
                ...
                /*
                // if either entity moves on the y-axis
                var minY = ( this.pos.y + this.size.y ) - 4;
                var maxY = ( this.pos.y + this.size.y ) + 4;
                var posY = ( this.pushingEntity.pos.y + this.pushingEntity.size.y );
                console.log(minY, posY, maxY);
                if ( posY < minY || posY > maxY ) {
                    console.log('someone fell');
                    this.pushing = false;
                }
                */
                if ( ! this.pushingEntity.isPushable ) {
                    this.pushing = false;
                }
                
                // if action button is released
            ...
            // if action button pressed while colliding with pushable enitity
            if ( this.canPush && ig.input.pressed('action') ) {
                this.pushing = true;
                this.pushingEntity = this.canPush;
                this.pushSpeed = this.canPush.pushSpeed;
            }
            
            // clean up parameters if not pushing
            if ( ! this.pushing ) {
                this.pushSpeed = 0;
                this.pushingEntity = false;
            }
        ...
        // called by overlapped entity if it can be pushed
        collideWithPushable: function( other ) {
            this.canPush = other;
        },
        ...
        handleMovementTrace: function( res ) {
            ...
            // stop pushing if hitting a wall
            if ( res.collision.x && this.pushing ) {
                this.pushing = false;
            }
        },
