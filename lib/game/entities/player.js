...
.defines(function() {
    EntityPlayer = ig.Entity.extend({
        ...
        init: function( x, y, settings ) {
            ...
            this.addAnim( 'push_idle', 1, [32], true );
            this.addAnim( 'push', 0.5, [33, 34, 33, 32] );
        ...
        update: function() {
            ...
            // reset pushing
            //this.pushing = false;
            //this.pushingEntity = false;
            
            // reset colliding with
        ...
        checkStatus: function() {
            ...
            if ( ! this.hurting && ! this.dying && ! this.pushing && ! this.pulling ) {
        ...
        isCrouching: function() {
            ...
            if ( this.hurting || this.dying || this.jumping || this.falling || this.attacking || this.pushing || this.pulling ) {
                return;
            }
        ...
        isPushing: function() {
            ...
            /*
            // if pushing
            if ( this.pushing && this.pushingEntity ) {
            
                // if the player is to the left of the entity
                if ( this.pos.x < this.pushingEntity.pos.x ) {
                    
                    // if pushing entity to the right
                    if ( ig.input.state('right') ) {
                        this.pushingEntity.beingPushed( this, 'right' );
                    }
                    
                    // else, if pulling the entity to the left
                        ...
                        if ( ! ig.game.collisionMap.getTile( xPos, yPos ) ) {
                            this.pushingEntity.beingPulled( this, 'left' );
                        }
                    }
                    
                }
                
                // else, if the player is to the right
                else if ( this.pos.x > this.pushingEntity.pos.x ) {
                    
                    // if pushing the entity to the left
                    if ( ig.input.state('left') ) {
                        this.pushingEntity.beingPushed( this, 'left' );
                    }
                    
                    // else, if pulling the entity to the right
                    else if ( ig.input.state('right') ) {
                        ...
                        var yPos = ( this.pos.y + this.size.y - 1 );
                        if ( ! ig.game.collisionMap.getTile( xPos, yPos ) ) {
                            this.pushingEntity.beingPulled( this, 'right' );
                        }
                    }
            ...
            */
            ...
            // if pushing and attack button is released
            if ( this.pushing && ig.input.released('attack') ) {
                this.pushing = false;
                this.pushingEntity = false;
            }
            
            // if attack button pressed while colliding
            if ( this.isCollidingWith && ig.input.pressed('attack') ) {
                this.pushing = true;
                this.pushingEntity = this.isCollidingWith;
                this.pushSpeed = this.isCollidingWith.pushSpeed;
            }
        ...
        isPulling: function() {
            ...
            // if pulling
            if ( this.pulling && this.pullingEntity ) {
                
                // if the player is pulling from the left side
                if ( this.pos.x < this.pullingEntity.pos.x ) {
                    if ( ig.input.state('left') ) {
                        ...
                        var yPos = ( this.pos.y + this.size.y - 1 );
                        if ( ! ig.game.collisionMap.getTile( xPos, yPos ) ) {
                            this.pullingEntity.beingPulled( this, 'left' );
                        }
                        
                    }
                }
                
                // else, the player is pulling from the right side
                else if ( this.pos.x > this.pullingEntity.pos.x ) {
                    if ( ig.input.state('right') ) {
                        ...
                        var yPos = ( this.pos.y + this.size.y - 1 );
                        if ( ! ig.game.collisionMap.getTile( xPos, yPos ) ) {
                            this.pullingEntity.beingPulled( this, 'right' );
                        }
            ...
            // if pulling and attack button is released
            if ( this.pulling && ig.input.released('attack') ) {
                this.pulling = false;
                this.pullingEntity = false;
            }
            
            // if attack button pressed while colliding
            if ( this.pushing && ig.input.pressed('attack') ) {
                if ( this.pushingEntity ) {
                    this.pulling = true;
                    this.pullingEntity = this.isCollidingWith;
                }
            }
            */
        },
        ...
        isAttacking: function() {
            ...
            if ( this.hurting || this.dying || this.crouching || this.pushing || this.pulling ) {
        ...
        isMoving: function() {
            ...
            // if pushing, change walk speed
            if ( this.pushing ) {
                if ( this.vel.x < 0 ) {
                    this.vel.x = -this.pushSpeed;
                }
                else if ( this.vel.x > 0 ) {
                    this.vel.x = this.pushSpeed;
                }
                ...
                if ( this.pushingEntity ) {
                    this.pushingEntity.pushing = true;
                    this.pushingEntity.pushingSpeed = this.vel.x;
                }
            }
        ...
        animate: function() {
            ...
            else if ( this.pushing || this.pulling ) {
                if ( this.walking ) {
                    if ( this.currentAnim != this.anims.push ) {
                        this.currentAnim = this.anims.push.rewind();
                    }
                } else {
                    if ( this.currentAnim != this.anims.push_idle ) {
                        this.currentAnim = this.anims.push_idle.rewind();
                    }
                }
            }
        ...
        collideWith: function( other, axis ) {
            ...
            /*
            if ( other instanceof EntitySnailGuard ) {
                if ( this.standing && axis == 'x' ) {
                    if ( other.hiding ) {
                        
                        // tell the player they are "pushing"
                        this.pushing = true;
                        this.entityPulling = other;
                        ...
                        if ( this.pos.x < other.pos.x ) {
                            ...
                            // check for a wall to the right
                            var xPos = ( other.pos.x + this.size.x + 1 );
                            var yPos = ( other.pos.y + this.size.y - 1 );
                            if ( ! ig.game.collisionMap.getTile( xPos, yPos ) ) {
                                other.pos.x += mx; // move to the right
                            }
                        }
                        ...
                        // else, the player is pushing from the right side
                            ...
                            // check for a wall to the left
                            var xPos = ( other.pos.x + - 1 );
                            var yPos = ( other.pos.y + this.size.y - 1 );
                            if ( ! ig.game.collisionMap.getTile( xPos, yPos ) ) {
                                other.pos.x -= mx; // move to the left
                            }
                        }
        ...
        handleMovementTrace: function( res ) {
            ...
            if ( res.collision.x ) {
                if ( this.pushing ) {
                    if ( this.pushingEntity ) {
                        this.pushingEntity.pushingSpeed = 0;
                    }
                }
            }
        },
